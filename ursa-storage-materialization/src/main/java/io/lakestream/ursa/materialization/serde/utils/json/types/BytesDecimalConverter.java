        ...
        try {
            ...
            if (silently) {
                ...
                throw new AvroTypeException("Field " + print(path)
                                            + " is expected to be a valid number. current value is " + value + ".");
            }
        }
    }
    ...
    protected BigDecimal bigDecimalWithExpectedScale(String decimal, int scale, Deque<String> path) {
        ...
        if (bigDecimalInput.scale() <= scale) {
            ...
            throw new AvroTypeException("Field " + print(path) + " is expected to be a number with scale up to "
                                        + scale + ". current value: " + bigDecimalInput + " is number with scale "
                                        + bigDecimalInput.scale() + ".");
        }
    }

    @Override
    public boolean canManage(Schema schema, Object jsonValue, Deque<String> deque) {
