...
import io.lakestream.ursa.lakehouse.iceberg.gcp.bigquery.BigQueryClientImpl;
...
public final class BigQueryMetastoreCatalog extends BaseMetastoreCatalog
  ...
  public void initialize(String inputName, Map<String, String> properties) {
    ...
    this.location = properties.getOrDefault(BIGQUERY_LOCATION, DEFAULT_GCP_LOCATION);
  ...
  void initialize(
    ...
    this.client =
        checkNotNull(bigQueryClient, "Failed to initialize BigQuery Client");
  ...
  public void renameTable(TableIdentifier from, TableIdentifier to) {
    ...
    // TODO: Enable once supported by the API.
    throw new ServiceFailureException(
        "Table rename operation is unsupported. Try the SQL operation directly on BigQuery: "
  ...
  private String getDefaultStorageLocationUri(String dbId) {
    checkNotNull(defaultWarehouseLocation, "Data warehouse location is not set");
    return String.format(
  ...
  private static void validateNamespace(Namespace namespace) {
    checkArgument(namespace.levels().length == 1, invalidNamespaceMessage(namespace));
  }
