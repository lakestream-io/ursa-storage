 ...
 * under the License.
 */
package io.lakestream.ursa.lakehouse.iceberg.gcp.bigquery;
...
public final class BigQueryClientImpl implements BigQueryClient {
    ...
    try {
      ...
      HttpResponse response =
          client
              .tables()
              .insert(
                  checkNotNull(table.getTableReference()).getProjectId(),
                  checkNotNull(table.getTableReference()).getDatasetId(),
                  table)
              .executeUnparsed();
  ...
  public Table patchTable(TableReference tableReference, Table table) {
    ...
    Table patch =
        new Table()
            ...
            // TODO: Update this once the server side accepts schema.
  ...
  public List<Tables> listTables(
    ...
    try {
      ...
      // TODO: The server should return more metadata here to distinguish Iceberg
    ...
    try {
      HttpResponse response =
          client
              .datasets()
              .update(
                  checkNotNull(dataset.getDatasetReference()).getProjectId(),
                  checkNotNull(dataset.getDatasetReference().getDatasetId()),
                  dataset)
