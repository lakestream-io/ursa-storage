...
public final class BigQueryTableOperations extends BaseMetastoreTableOperations {
  ...
  // parameter.
  //
  // TODO: Decide on whether and how to make the table queryable from Hive engine.
  ...
  private static void updateParametersWithSnapshotMetadata(
    ...
    if (summary.get(SnapshotSummary.TOTAL_DATA_FILES_PROP) != null) {
      parameters.put(NUM_FILES, summary.get(SnapshotSummary.TOTAL_DATA_FILES_PROP));
    }

    if (summary.get(SnapshotSummary.TOTAL_RECORDS_PROP) != null) {
      parameters.put(ROW_COUNT, summary.get(SnapshotSummary.TOTAL_RECORDS_PROP));
    }

    if (summary.get(SnapshotSummary.TOTAL_FILE_SIZE_PROP) != null) {
      parameters.put(TOTAL_SIZE, summary.get(SnapshotSummary.TOTAL_FILE_SIZE_PROP));
    }
  }
